...
document.getElementById("form-add-usuario").addEventListener("submit", function (event) {
    ...
    else if (validacionNuevoUsuarioBD(correo, tipoDoc, numeroDoc, usuario) !== "") {
        msg = validacionNuevoUsuarioBD(correo, tipoDoc, numeroDoc, usuario);
    }
    else {
...
document.getElementById("form-edit-usuario").addEventListener("submit", function (event) {
    ...
    else if (validacionEdicionUsuarioBD(correo, tipoDoc, numeroDoc, usuario, idUsuario) !== "") {
        msg = validacionEdicionUsuarioBD(correo, tipoDoc, numeroDoc, usuario, idUsuario);
    }
...
function validacionNuevoUsuarioBD(correo, tipoDoc, numeroDoc, usuario) {
    let msg = "";
    for (let i = 0; i < dataUsuariosBD.length; i++) {
        if (dataUsuariosBD[i].correo == correo) {
            msg = "El correo ingresado ya está registrado.";
        }
        else if (dataUsuariosBD[i].tipoDoc == tipoDoc && dataUsuariosBD[i].numeroDoc == numeroDoc) {
...
function validacionEdicionUsuarioBD(correo, tipoDoc, numeroDoc, usuario, id) {
    let msg = "";
    for (let i = 0; i < dataUsuariosBD.length; i++) {
